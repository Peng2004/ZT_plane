...
void april_callback(const detection_msgs::AprilTagDetectionArray::ConstPtr& msg){
    ...
    april_pos[1] = -(aprilDetection.pose.pose.pose.position.y) + 0.1;//摄像头偏差10cm

    cout << "april_pos:" << april_pos[0] << "  " << april_pos[1] << endl;
...
void x_adjustment(){
    ...
    // 使用if语句来替代switch
    if (april_pos_x >= 0.6) {
        flightByvel(0,1,0,0,50); // 位置大于或等于0.6
    } 
    else if (april_pos_x <= -0.6) {
        flightByvel(0,-1,0,0,50); // 位置小于或等于-0.6
    } 
    else if (april_pos_x >= 0.1 && april_pos_x < 0.3) {
        flightByvel(0,0.2,0,0,50); // 位置在0.1到0.3
    } 
    else if (april_pos_x > -0.3 && april_pos_x <= -0.1) {
        flightByvel(0,-0.2,0,0,50); // 位置在-0.3到-0.1
    } 
    else if (april_pos_x > 0.05 && april_pos_x <= 0.1) {
        flightByvel(0,0.1,0,0,50); // 位置在0.05到0.1
    } 
    else if (april_pos_x >= -0.1 && april_pos_x <= -0.05) {
        flightByvel(0,-0.1,0,0,50); // 位置在-0.1到-0.05
    } 
    else if (april_pos_x >= -0.05 && april_pos_x <= 0.05) {
        x_adjust_status = 1; // 位置在-0.05到0.05
        cout << "x_adjustment is done" << endl;
    }
}
...
void y_adjustment(){
    ...
    // 使用if语句来替代switch
    if (april_pos_y >= 0.6) {
        flightByvel(1,0,0,0,50); // 位置大于或等于0.6
    } 
    else if (april_pos_y <= -0.6) {
        flightByvel(-1,0,0,0,50); // 位置小于或等于-0.6
    } 
    else if (april_pos_y >= 0.1 && april_pos_y < 0.3) {
        flightByvel(0.2,0,0,0,50); // 位置在0.1到0.3
    } 
    else if (april_pos_y > -0.3 && april_pos_y <= -0.1) {
        flightByvel(-0.2,0,0,0,50); // 位置在-0.3到-0.1
    } 
    else if (april_pos_y > 0.05 && april_pos_y < 0.1) {
        flightByvel(0.1,0,0,0,50); // 位置在0.05到0.1
    } 
    else if (april_pos_y > -0.1 && april_pos_y <= -0.05) {
        flightByvel(-0.1,0,0,0,50); // 位置在-0.1到-0.05
    } 
    else if (april_pos_y > -0.05 && april_pos_y < 0.05) {
        y_adjust_status = 1; // 位置在-0.05到0.05
        cout << "y_adjustment is done" << endl;
    }
}
...
void task1(ros::NodeHandle n){
    ...
    //前往第二排货架    1.5m__2.6m
    // cin >> test_point;
    flightByvel(0,1,0,0,1300);//1300 750 1100
    ros::Duration(1).sleep();
    // cin >> test_point;
    flightByvel(1.5,0,0,0,1600); 
    ros::Duration(1).sleep();
...
int main(int argc, char** argv) {
    ...
    while(ros::ok()){ 
        ...
        cout << "1 - takeoff,2 - land,3 - flightByvel,4 - gimbalControl,5 - quit" << endl;
